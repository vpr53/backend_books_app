...
def get(request, title: str):
    ...
    try:
        ...
        return 502, {"error": "External API error", "details": str(e)}

    except Exception as e:
        return 500, {"error": "Unexpected error", "details": str(e)}


@api.post("/", auth=JWTAuth(), response={200: BookSchemaOut, 409: ErrorSchema})
...
@api.get("/", auth=JWTAuth(), response=List[BookSchemaOut])
def list_books(
    request,
...
@api.put("/", auth=JWTAuth(), response={200: BookSchemaOut, 409: ErrorSchema})
def update_book(request, book_id: int, payload: BookSchemaIn):
    ...
    if (
        ...
        and BookModels.objects.filter(google_id=payload.google_id)
        .exclude(id=book_id)
        .exists()
    ):
