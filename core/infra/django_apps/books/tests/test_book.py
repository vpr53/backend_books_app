...
class BookTest(BaseBookTestCase):
    ...
    def test_create_book_to_200(self):
        response = self.client.post(
            "/api/books/",
            data=self.book_payload(
                google_id="qfqiAAAAQBAK", title="Мастер и Маргарита"
            ),
        )
    ...
    def test_create_book_to_401(self):
        response = self.client.post(
            "/api/books/",
            data=json.dumps(
                self.book_payload(google_id="674291498137", title="Мастер и Маргарита1")
            ),
    ...
    def test_create_book_to_409(self):
        response = self.client.post(
            "/api/books/",
            data=self.book_payload(
                google_id=f"{self.book1.google_id}", title="Мастер и Маргарита1"
            ),
        )
        self.assertEqual(
            response.json()["detail"], "Book with this Google ID already exists"
        )
    ...
    def test_update_book_to_200(self):
        ...
        response = self.client.put(
            ...
            data=self.book_payload(
                google_id=f"{self.book1.google_id}", title="Мастер и Маргарита2"
            ),
        )
    ...
    def test_update_book_to_409(self):
        response = self.client.put(
            ...
            data=self.book_payload(
                google_id=f"{self.book2.google_id}", title="948u2348"
            ),
        )
    ...
    def test_update_book_to_401(self):
        response = self.client.put(
            f"/api/books/?book_id={self.book1.id}",
            data=json.dumps(
                self.book_payload(google_id=f"{self.book1.google_id}", title="948u2348")
            ),
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 401)
